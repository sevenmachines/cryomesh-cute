...
void PatternChannelTest::testPatternChannel() {
	// create a uniform list of patterns
	{
		...
		const std::list<boost::uuids::uuid>::iterator it_now =
				pc.getPatternListIterator();
...
void PatternChannelTest::testaddPatterns() {
	...
	//check patterns are added in order to end
	{
		...
		std::list<boost::uuids::uuid>::const_iterator it_pat_list =
				pat_list.begin();
		const std::list<boost::uuids::uuid>::const_iterator it_pat_list_end =
				pat_list.end();
...
void PatternChannelTest::testgetCurrentPattern() {
...

	{
		...
		bool eq = (current_pat->getUUID() == pat2->getUUID());
		ASSERT(eq);
	}
}
...
void PatternChannelTest::testgetPatternByTag(void) {
	...
	// get a pattern
	boost::shared_ptr<Pattern> test_pat = pc.getPatternByTag(
			pat2->getPatternTag());
...
boost::shared_ptr<PatternChannel> PatternChannelTest::getTestPatternChannel() {
	...
	patterns.push_back(pat5);
	// create channel
	boost::shared_ptr<PatternChannel> pc(new PatternChannel(patterns,
			PatternChannel::Input));
	return pc;
}
}
}
